...
int main(int argc, char* argv[]) {
  ...
  printf("Input Tensor id, name, type, shape: %i, %s, %s(%d), %s\n", in_id,
         in_name, TfLiteTypeGetName(in_type), in_type, in_shape);
  ...
  for (int i = 0; i < out_sz; i++) {
    ...
    auto t_type = t->type;
    printf("  %i, %s, %s(%d), %s\n", t_id, t->name, TfLiteTypeGetName(t_type),
           t_type, getShape(t).c_str());
  }
    ...
    if (out_sz == 4) {  // post-processed output
      ...
      if (t_type == 1) {  // float32
        ...
        for (int i = 0; i < n; i++) {
          ...
          printf(", box: %f, %f, %f, %f\n", output0[i * 4 + 0],
                 output0[i * 4 + 1], output0[i * 4 + 2], output0[i * 4 + 3]);
        }
      } else if (t_type == 3) {  // uint8
        unsigned char* output0 =
            interpreter->typed_output_tensor<unsigned char>(0);
        unsigned char* output1 =
            interpreter->typed_output_tensor<unsigned char>(1);
        unsigned char* output2 =
            interpreter->typed_output_tensor<unsigned char>(2);
        unsigned char* output3 =
            interpreter->typed_output_tensor<unsigned char>(3);
        int n = (int)(output3[0]);
        printf("num_of_objects: %d\n", n);
        for (int i = 0; i < n; i++) {
          printf("%d: ", i);
      ...
      for (int i = 0; i < out_sz; i++) {
        ...
        if (t_type == 1) {  // float32
          ...
          unsigned char* output =
              interpreter->typed_output_tensor<unsigned char>(i);
          printf("output[%d][0]: %d\n", i, output[0]);
        }
      }
    }
